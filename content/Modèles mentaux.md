---
title: 🗺️ Modèles mentaux
permalink: modeles-mentaux
description: Prenez de meilleures décisions
aliases:
  - Mental model
tags:
  - moc
datePublished: 2021-06-27T19:19
dateUpdated: 2024-11-16T19:19:00
publish: true
comments: false
rating: 4
note: 80
---

Un modèle mental est un concept, un système ou une manière de voir le monde qui nous aide à comprendre comment les choses fonctionnent et qui peut être appliqué à plusieurs domaines.

C'est un concept qui peut être appliqué dans plusieurs domaines et qui nous aide à comprendre le monde.

> [!QUOTE] Abraham Maslow
> Si tout ce que vous avez est un marteau, tout ressemble à un clou.

> [!QUOTE] Michael Simmons
> Un modèle mental est un principe que vous pouvez appliquer dans plusieurs domaines.

> [!QUOTE] [George Mack](https://youtu.be/m_56L8EGLIk?t=7)
> Votre cerveau est un système d'exploitation et les modèles mentaux sont des applications que vous pouvez installer.

- [[L'effet Lindy]]
- [[Loi de Pareto]]
- [[First Principles]]
- [[La Théorie des Contraintes]]
- [[Loi de Gall]]
- [[Via negativa]]
- [[Second-Order Thinking]]
- [[Simplification de causalité]]
- [[Le principe d'inversion]]
- [[Méthode scientifique]]
- [[Loi de Parkinson]]
- [[Contraste]]
- [[Game theory]]
- [[Correlation vs. Causalité]]
- [[The Paradox of Choice]]
- [[Facteurs d'influence]]
- [[La carte n'est pas le territoire]]
- [[La pensée illusoire]]
- [[Table Selection]]

---

Références :

- [Livres sur les modèles mentaux](https://goodreads.com/shelf/show/mental-models)
- [Farnam Street: Mental Models](https://vimeo.com/177585900)
- [Marketing Mania - Modèles Mentaux du Marketeur](https://marketingmania.fr/modeles-mentaux/)
- [Valentin Decker - Les modèles mentaux](https://medium.com/latelier-verrocchio/e814d1b3b3dc)
- [The 10x Engineer - A Tool kit of mental models](https://medium.com/10x-curiosity/b9781d9e1cb2)
- [Michael Simmons - The Top 12 Most Useful & Universal Mental Models](https://i.imgur.com/70Hwkhj.jpg)
- [George MacGill : Comment prendre de meilleures décisions](https://medium.com/essentiels/f41cb604d3d0)
- [The Latticework: Interconnecting the Big Ideas from the Big Disciplines](https://ltcwrk.com/)
- [Julian Shapiro - Mental models](https://julian.com/blog/mental-model-examples)
- [Modèles Mentaux #1 : L'idée qui valait 1 milliard](https://youtube.com/watch?v=geyT-7ApNRQ)
- [Everlaab - Liste de modèles mentaux pour mieux raisonner](https://everlaab.com/liste-de-modeles-mentaux/)
- [ModelThinkers - Be Smarter. Faster](https://modelthinkers.com/)
- [Modèles mentaux : la (seule) manière de prendre des décisions intelligentes](https://youtu.be/B5ZW0UGuzu4)
- [James Clear - Mental Models](https://jamesclear.com/mental-models)
- [Buy a deck of mental models](https://wisecharlie.com/)
- [30 mental models to add to your thinking toolbox](https://nesslabs.com/mental-models)
- [40 powerful concepts for understanding the world](https://twitter.com/G_S_Bhogal/status/1225561131122597896)
- [40 useful concepts you should know](https://twitter.com/G_S_Bhogal/status/1492255231169679365)
- [Kevin Kirkpatrick - Path Nine](https://pathnine.substack.com/)
- [Laws, Theories, Principles and Patterns that developers will find useful](https://github.com/dwmkerr/hacker-laws)
- [BJJ Mental Models](https://bjjmentalmodels.com/database/)
- [Oreilly - The Programmer's Brain](https://oreilly.com/library/view/the-programmers-brain/9781617298677/)
- [Knowledge Project - Josh Kaufman: Mental Models Lead to a Successful Business](https://youtu.be/RgP9MUFGNuk)
- https://ltcwrk.com/
- https://brainytab.com/
- https://everlaab.podia.com/master-modele-mental
- https://theodorusclarence.com/blog/btb-flex-mental-model
- https://medium.com/@yegg/mental-models-i-find-repeatedly-useful-936f1cc405d
- https://commoncog.com/the-mental-model-fallacy/
- https://medium.com/essentiels/5d790b64667
- https://junto.investments/mental-models/
- https://docs.google.com/spreadsheets/d/1wng4d-Pl4rvKcxQCsxNX0bUdY-8iPQuBAXTQQYg9H14/edit
- https://i.imgur.com/AwYc6TA.jpg
- https://mentalmodels.co
- One Pager Compilation - Blas Moros.pdf