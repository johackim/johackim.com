---
title: WOOP Framework
links: "[[Goal setting]]"
permalink: woop-framework
aliases: [WOOP Method]
description: Découvrez le système WOOP pour atteindre vos objectifs en 4 étapes ; établir un souhait, déterminer les résultats, identifier les obstacles et planifier.
datePublished: 2021-10-06T16:50
dateUpdated: 2021-10-06T16:50
publish: true
rss: true
note: 64
---

Le système WOOP est composé de 4 parties :

1. **Wish** : *Définissez un rêve/objectif* - Que voulez-vous ? Décrivez-le en quelques mots.

2. **Outcomes** : *Quels bénéfices/ressentis allez-vous obtenir en réalisant cet objectif ?* - Décrivez-les

3. **Obstacles** : *Quels obstacles allez-vous rencontrer ?* - Affrontez le réel et concentrez-vous sur les obstacles qui vous se dresser devant vous. Listez-les.

4. **Plans :** *Préparez-le(s) plan(s) nécessaire(s) avec la méthode : Si obstacle (X) alors plan (Y).* C’est ce que l’auteur appelle **l’implémentation d’intentions.** Choisissez le plan le plus efficace.

---

Références :

- https://woopmylife.org/fr/home
- https://everlaab.com/methode-woop/
- https://jeancharleskurdali.com/le-systeme-en-4-etapes-que-vous-devez-absolument-connaitre-et-utiliser-pour-atteindre-vos-objectifs/
- [Rethinking Positive Thinking - Gabriele Oettingen](https://amzn.to/3p6qNN3)