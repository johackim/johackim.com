...
export const getContentList = async () => fs.readdirSync(CONTENT_FOLDER).filter((f) => f.endsWith('.md')).map((file) => {
...
export const renderMarkdownToHtml = (markdown, markdownFiles, components) => renderToStaticMarkup(
...
export const createRss = async () => {
    ...
    const items = await Promise.all(contents.map(async ({ fileName, permalink, title, description, datePublished }) => {
        ...
        return { fileName, permalink, title, description, datePublished, contentHtml: renderMarkdownToHtml(markdown) };
    }));
    ...
    items.forEach(({ fileName, permalink, title, description, datePublished, contentHtml }) => {
        feed.item({
            ...
            custom_elements: [
                ...
                { 'content:encoded': { _cdata: removeSvg(contentHtml).replace(/href="\//g, 'href="https://johackim.com/') } },
            ],
        });
    });
...
export const createNotes = async () => {
    const contents = (await getContentList())
        .filter(({ file }) => {
            const body = fs.readFileSync(`${CONTENT_FOLDER}/${file}`, 'utf-8').replace(/^---\n[\s\S]+?\n---\n?/, '').trim();
            return body.length <= 280;
        })
        .sort((a, b) => new Date(b.datePublished) - new Date(a.datePublished));

    const feed = new RSS({
        title: 'Johackim — Notes',
        description: 'Notes courtes syndiquées (POSSE)',
        site_url: 'https://johackim.com',
        generator: 'Next.js',
        custom_elements: [
            { 'atom:link': { _attr: { href: 'https://johackim.com/notes.xml', rel: 'self', type: 'application/rss+xml' } } },
        ],
    });

    contents.forEach(({ file, fileName, permalink, datePublished }) => {
        const raw = fs.readFileSync(`${CONTENT_FOLDER}/${file}`, 'utf-8');
    ...
    fs.writeFileSync('public/notes.xml', feed.xml({ indent: true }));
};
