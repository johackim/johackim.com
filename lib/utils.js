...
const extractFrontmatter = (file) => {
    ...
    const slug = String(frontmatter?.slug || slugify(fileName, { lower: true }));
...
export const getContents = async () => cache(async () => {
    ...
    files = files.filter(({ datePublished }) => {
        const currentDate = new Date(new Date().getTime() + (new Date().getTimezoneOffset() * 60000) + (3600000 * 1));
        return new Date(datePublished) <= currentDate;
    });
...
export const getTwitterFollowers = async () => {
    const data = await fetch('https://n8n.ethibox.fr/webhook/7b89a655-3752-4b84-838c-6792dcb0970b').then((res) => res.json());
