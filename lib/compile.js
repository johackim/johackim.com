...
export const compileMdxToJs = async (content, enablePaywall = true) => {
    const contents = await getContents();

    const titleToUrl = (title) => {
        const fileName = `${title}.md`;
        const file = contents.find((f) => basename(f.fileName) === fileName);
    ...
    const paywall = enablePaywall ? ReactDOMServer.renderToStaticMarkup(<Reserved />) : false;
